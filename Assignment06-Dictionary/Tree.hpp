...
//
// NOTE ON THE PARENT POINTER ADDED BELOW:
// Because this tree is persistent/immutable, a Node can end up shared by more
// than one Tree at once (insert() reuses whichever subtrees didn't change).
// That means "the" parent of a node isn't really well-defined in general --
// it depends on which Tree you're looking at it through. The _parent field
// here reflects the most recently-built Tree that this node was attached
// to as a child, which is enough to walk "up" from a node you reached by
// walking down through left()/right() calls on a single Tree, but it is not
// a strict invariant of the whole structure the way _lft/_rgt are. It's
// stored as a weak_ptr (not shared_ptr) specifically so that parent/child
// links don't form an ownership cycle that would leak memory.
//
#pragma once
...
class Tree
{
    ...
    struct Node
    {
        ...
        // mutable because we need to set it through a shared_ptr<const Node>
        // -- the node's value/children are logically immutable, but the
        // parent link is bookkeeping we attach after the node already exists.
...
public:
    ...
    {
        // Wire up the parent pointer on whichever subtrees we were just
        // handed. The parent of the root of the whole tree is left unset
        // (an expired weak_ptr), which is exactly the "nullptr" behavior
        // the assignment asks for.
        if (lft._root) lft._root->_parent = _root;
        if (rgt._root) rgt._root->_parent = _root;
    }
    ...
    // Returns the parent of this node, per the most recently-built Tree it
    // was attached to (see the big note at the top of the file). The parent
    // of the root is an empty Tree, i.e. isEmpty() == true, matching how the
    // assignment describes "the parent of the root node is set to nullptr".
    Tree parent() const {
        assert(!isEmpty());
...
//
// levelTraversal: a stand-alone (non-member) recursive function that visits
// a Tree level by level -- root first (level 0), then all of level 1, then
// all of level 2, and so on.
//
// The "tracking mechanism" hinted at in the assignment is a queue: we push
// the root, then repeatedly pop the front node, visit it, and push its
// children (left before right) onto the back of the queue. That's what
// guarantees level-by-level order -- everything at a shallower level is
// always popped before anything at a deeper level.
//
// It's written recursively (as asked) by having the queue itself be the
// recursive state: each call processes exactly one node off the front and
// recurses with the updated queue, instead of using an explicit while loop.
//
template<typename T>
void levelTraversal(std::queue<Tree<T>> & nodesToVisit, std::function<void(T)> visit) {
    if (nodesToVisit.empty()) return;
    ...
    if (!current.left().isEmpty())  nodesToVisit.push(current.left());
    if (!current.right().isEmpty()) nodesToVisit.push(current.right());

    levelTraversal(nodesToVisit, visit);
}

template<typename T>
void levelTraversal(Tree<T> & tree, std::function<void(T)> visit) {
...
//
// prune: a stand-alone (non-member) recursive function that takes a Tree by
// reference and returns a new Tree that's a copy of the original except
// that none of the ORIGINAL tree's leaf nodes are present.
//
// "Leaf" is judged against the tree as it was passed in, not against
// whatever's left after pruning -- so a node that only becomes childless
// because both its children got pruned away is NOT itself removed. Only
// nodes that were already leaves in the tree you started with disappear.
//
template<typename T>
Tree<T> prune(const Tree<T> & tree) {
    if (tree.isEmpty()) return Tree<T>();

    bool isLeaf = tree.left().isEmpty() && tree.right().isEmpty();
    if (isLeaf) {
        return Tree<T>(); // this node was a leaf in the original tree -- drop it
    }
